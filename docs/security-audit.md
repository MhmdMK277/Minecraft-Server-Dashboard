...
| F11 | Medium | The new firewall check (server/reachability.ts, built 2026-08-06 for the Addresses page) matched ANY enabled inbound allow rule covering the port, including app-scoped rules: its first live run declared a Minecraft server's port open on the strength of Microsoft Teams' any-port rule, which opens the port for Teams and nobody else. The check would have told a user their firewall was fine while every outside connection was still blocked: the F6/F7 class exactly, a confident reading about reachability with nothing behind it, caught only because the proof runs against the real machine's rule store and printed the matched rule name | **Fixed** the same hour, before the check ever shipped: an app-scoped rule counts only when its program is a Java executable; prove-addresses pins that another program's any-port rule never covers the server and a java-scoped one does |
